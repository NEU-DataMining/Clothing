...
//保存图像
void GCApplication::saveImage(string str)
{
    if( image->empty() || winName->empty() )
        return;
	...
	cvSaveImage(s, &imgipl);

	IplImage* img = cvCreateImage(cvSize(600,600),8,3);cvZero(img);
	CvFont font;
	cvInitFont(&font,CV_FONT_HERSHEY_SIMPLEX|CV_FONT_ITALIC,1.0,1.0,0,1);
	cvPutText(img,"Save!",cvPoint(600,600),&font,cvScalar(255,255,0)); 
	//cvShowImage( "gcout", img);
	cvWaitKey(0);
	//cvDestroyWindow( "gcout");
}
